...
public class IdentityHashMapImpl  implements CollectionImpl {

    private IdentityHashMap<String, String> mIdentityHashMap = new IdentityHashMap();
    ...
    public void prepareData() {
        ...
        for (String arrDatum : arrData) {
            mIdentityHashMap.put(arrDatum, arrDatum);
        }
    }

    @Override
    public void checkNullAllowed() {
        mIdentityHashMap.put(null, "nullKey 1");
        mIdentityHashMap.put(null, "nullKey 2"); // will overwrite the above value of above statement
        mIdentityHashMap.put(new String(arrData[0]), null);
        mIdentityHashMap.put(new String(arrData[1]), null);

        Iterator itr = mIdentityHashMap.entrySet().iterator();
        while (itr.hasNext()) {
    ...
    public void checkOrder() {
        for(int i = 1; i < 5; i ++) {
            ...
            for (Map.Entry entry: mIdentityHashMap.entrySet()) {
                print(ConsoleColors.BLUE, "KEY = " + entry.getKey() +" " + "VALUE = " + entry.getValue());
            }
        }
    }
        ...
        for (int i = 1; i < 5; i++) {
            ...
            timer.scheduleAtFixedRate(new TimerTask() {
                ...
                public void run() {
                    ...
                    mIdentityHashMap.put("element_" + timerCount, "element_" + timerCount);
    ...
    public void specialFunctions() {
        ...
        mIdentityHashMap.putIfAbsent("A", "A"); // do no insert if key already has value, but for "A" we have null as value so it will be updated.
        mIdentityHashMap.putIfAbsent(null, "@@"); // do no insert if key already has value, for null here we already have value so it will be skipped.
        print(ConsoleColors.BLACK_BOLD, "[ Put if absent ]");
        iterateUsingWhile("");

        mIdentityHashMap.replace("A", "##");
        print(ConsoleColors.BLACK_BOLD, "[ simple replace ]");
        iterateUsingWhile("");

        mIdentityHashMap.replace("B", "A", "@@A@@");
        print(ConsoleColors.BLACK_BOLD, "[ replace with wrong old value ]");
        iterateUsingWhile("");

        mIdentityHashMap.replace("C", "C", "@@C@@");
        print(ConsoleColors.BLACK_BOLD, "[ replace with right old value ]");
        iterateUsingWhile("");


        print(ConsoleColors.BLACK_BOLD, "[ compute ]");
        mIdentityHashMap.compute("C", (K, V) ->
                (V != null && V.contains("@") ? V = K : "")); // manipulation to bne done here on K or V only
        // mIdentityHashMap.replace() wont work as may be it works on clone copy.

        iterateUsingWhile("");
        //mIdentityHashMap.computeIfAbsent();
        //mIdentityHashMap.computeIfPresent();
    ...
    public void iterationUsingForEachRemaining() {
        ...
        mIdentityHashMap.entrySet().iterator().forEachRemaining(entry ->
        {
            print(ConsoleColors.BLUE, "KEY = " + entry.getKey() +" " + "VALUE = " + entry.getValue());
        });
    }
    ...
    public void iterateUsingStream() {
        ...
        Stream<Map.Entry<String, String>> stream = mIdentityHashMap.entrySet().stream();
